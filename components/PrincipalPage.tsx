...
const DEFAULTS = { totalCarimbos: 8, premio: "Combo Speed Master" };

export default function PrincipalPage() {
  const router = useRouter();
  const [cliente, setCliente] = useState<ClienteData | null>(null);
  const [totalCartela, setTotalCartela] = useState(DEFAULTS.totalCarimbos);
  const [premio, setPremio] = useState(DEFAULTS.premio);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        .from("configuracoes")
        .select("premio, carimbos_necessarios")
        .maybeSingle(),
    ]).then(([{ data: clienteData }, { data: config, error: configError }]) => {
      const c = clienteData ?? clienteStored;
      setCliente(c as ClienteData);
      sessionStorage.setItem("cliente", JSON.stringify(c));

      if (config) {
        if (config.carimbos_necessarios != null)
          setTotalCartela(config.carimbos_necessarios);
        if (config.premio != null) setPremio(config.premio);
      }
    });
  }, [router]);
  ...
  const confirmarResgate = async () => {
    ...
    const novaQtd = cliente.quantidade_carimbos - totalCartela;
  ...
  const cartelaCompleta = cliente.quantidade_carimbos >= totalCartela;
  const temResgatePendente = (cliente.resgates_disponiveis || 0) > 0;
    ...
    <ScreenBackground>
      <div className="mx-auto max-w-lg flex flex-col items-center px-6">
        <div className="bg-black/40 rounded-[3em] shadow-xl w-full flex flex-col border border-orange-500/80 p-8 gap-6">
          ...
          <CodigoCarimbo
            cliente={cliente}
            totalCartela={totalCartela}
            onCarimboAdicionado={handleClienteAtualizado}
          />

          <Instrucoes totalCarimbos={totalCartela} premio={premio} />
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 w-full max-w-sm bg-zinc-950 border border-orange-500/60 rounded-[2rem] p-6 shadow-2xl">
            ...
            <p className="text-zinc-300 text-sm mb-2">
              Você está prestes a converter seus selos em um{" "}
              <span className="text-orange-400 font-semibold">{premio}</span>!
            </p>
            <p className="text-zinc-300 text-sm mb-6">
              Selos atuais:{" "}
              <span className="text-orange-400 font-semibold">
                {cliente.quantidade_carimbos}
              </span>
              {cliente.quantidade_carimbos > totalCartela && (
            ...
            {mensagemResgate && (
              <p
                className={`text-xs font-semibold mb-3 ${mensagemResgate.includes("Erro") ? "text-red-400" : "text-zinc-400"}`}
              >
                {mensagemResgate}
              </p>
            )}

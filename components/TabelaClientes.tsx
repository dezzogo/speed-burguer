...
import { useRef, useState } from "react";import { supabase } from "../src/supabase";
...
export default function TabelaClientes({ clientes, onClientesChange }: TabelaClientesProps) {
  ...
  const salvarEdicao = async () => {
    ...
    const { error } = await supabase
      ...
      .update({ nome_cliente: nomeTrimmed, telefone: telTrimmed, quantidade_carimbos: editCarimbos })
      .eq("id", modal.cliente.id);
    if (error) { setModalMensagem("Erro ao salvar."); return; }
    onClientesChange(
      clientes.map((c) =>
        c.id === modal.cliente!.id
          ? { ...c, nome_cliente: nomeTrimmed, telefone: telTrimmed, quantidade_carimbos: editCarimbos }
          : c
      )
    );
    fecharModal();
  ...
  const consumirCartela = async () => {
    if (!modal.cliente) return;
    const atual = modal.cliente.quantidade_carimbos;
    if (atual < TOTAL_CARTELA) {
      setModalMensagem(`O cliente ainda não completou os ${TOTAL_CARTELA} selos.`);
      return;
    }
    const novaQtd = atual - TOTAL_CARTELA;
    const { error } = await supabase
      .from("cartoes_fidelidade")
      .update({ quantidade_carimbos: novaQtd })
      .eq("id", modal.cliente.id);
    if (error) { setModalMensagem("Erro ao consumir cartela."); return; }
    onClientesChange(
      clientes.map((c) =>
        c.id === modal.cliente!.id ? { ...c, quantidade_carimbos: novaQtd } : c
      )
    );
    fecharModal();
  };
  ...
  return (
    <>
      ...
      <div className="rounded-[2rem] border border-zinc-800 bg-black overflow-clip" ref={menuRef}>
        {/* Cabeçalho desktop */}
        <div className="hidden md:grid md:grid-cols-[2fr_1fr_1fr_40px] px-6 py-4 border-b border-zinc-800">
        ...
        {clientesFiltrados.map((cliente, index) => {
          ...
          <div key={cliente.id} className="border-b border-zinc-900 last:border-0 relative">
            ...
            <div className="md:hidden px-5 py-4 flex items-center gap-4">
              ...
              <span className="ml-auto text-zinc-300 text-sm shrink-0">🍔 {cliente.quantidade_carimbos}</span>
              <MenuBotao
            ...
            {/* Desktop */}
            <div className="hidden md:grid md:grid-cols-[2fr_1fr_1fr_40px] px-6 py-5 items-center">
      ...
      {modal.tipo === "editar" && modal.cliente && (
        <Modal titulo="Editar cliente" onFechar={fecharModal}>
          ...
          <input value={editNome} onChange={(e) => setEditNome(e.target.value)}
            className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-orange-500 mb-4" />
          ...
          <input value={editTelefone} onChange={(e) => setEditTelefone(e.target.value)}
            className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-orange-500 mb-4" />
          <label className="text-zinc-400 text-xs mb-1">Quantidade de carimbos</label>
      ...
      {modal.tipo === "consumir" && modal.cliente && (
        <Modal titulo="Consumir cartela" onFechar={fecharModal}>
          <p className="text-zinc-300 text-sm mb-2">
            Cliente: <span className="text-white font-semibold">{modal.cliente.nome_cliente}</span>
          </p>
          <p className="text-zinc-300 text-sm mb-6">
            Selos atuais:{" "}
            <span className="text-orange-400 font-semibold">{modal.cliente.quantidade_carimbos}</span>
            {modal.cliente.quantidade_carimbos >= TOTAL_CARTELA && (
          ...
          <div className="flex gap-3">
            <button onClick={fecharModal} className="flex-1 py-3 rounded-xl border border-zinc-700 text-zinc-400 hover:text-white transition-colors text-sm">Cancelar</button>
            <button onClick={consumirCartela} className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white font-bold text-sm transition-colors">Confirmar</button>
          </div>
        </Modal>
      )}
    </>
...
function MenuBotao({ aberto, abrirParaCima, onToggle, onEditar, onExcluir, onConsumir }: MenuBotaoProps) {
  return (
    <div className="relative">
      ...
      {aberto && (
        <div
          className="absolute right-0 z-50 w-44 bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl overflow-hidden"
